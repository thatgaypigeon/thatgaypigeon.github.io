...
def main() -> list[str]:
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        ...
        while i < len(file_contents):
            index += 1
            ling_orig: str = file_contents[i]
            line: str = ling_orig.strip()
            if re.fullmatch(r"^\/\/ -* .* -* \/\/$", line):
                ...
                if line_len == major_len:
                    ...
                    title = title.title()
                    if isinstance(contents[prev_major], dict):
                    ...
                    title = title.title()
                    if isinstance(contents[prev_major][prev_minor], dict):
            ...
            # Check mini-headers
            elif re.match(r"^\/\* .* \*\/$", ling_orig) and not re.fullmatch(
                r"^\/", file_contents[i + 1]
            ):
                file_contents[
                    i
                ] = f"/* {BULLET} {' '.join(ling_orig.split(' ')[1:-1]).lstrip(BULLET).lstrip('->').strip()}\n// {'-' * 21} */\n"
            ...
            # Format mini-headers
                ...
                file_contents[
                    i
                ] = f"/* {BULLET} {sentenceCase(' '.join(ling_orig.split(' ')[1:]).lstrip(BULLET).lstrip('->').strip())}\n"
